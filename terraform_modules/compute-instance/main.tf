...
resource "oci_core_instance" "instance" {
  ...
  //fault_domain         = var.instance_fault_domain == true && var.instance_count <= "3" ? "FAULT-DOMAIN-${count.index + 1}" : "${ element(local.FDs, count.index)}"
  // If fault_domain is true, spread instances on all FDs in round-robin. Looping to the first when last FD is reached, Otherwise use FD1
  fault_domain         = var.instance_fault_domain == true ?  element(local.FDs, count.index) : "FAULT-DOMAIN-1"
  compartment_id       = var.compartment_ocid
